    ...
    <>{
      showAnnouncements &&
        ...
        style={{ backgroundColor: color.background, color: color.foreground }}
          className="flex flex-row relative text-center justify-center px-7 md:px-8 py-2 shadow">
            <p className="text-xs md:text-sm">
            I'm migrating to a new interface—currently cooking! 🍳 But you can take a peek:{" "} <a href={'https://devrahmlad.vercel.app'} target="_blank" className="underline underline-offset-2 inline-flex" rel="noreferrer">devrahmlad.vercel.app</a> 👀
            </p>
            <button
                onClick={()=>setShowAnnouncements(!showAnnouncements)}
                className="flex absolute right-0 top-0 p-3 ease-in transition duration-500 active:scale-90"
              >
                <FaTimes size={15} />
              </button>
          </div>
